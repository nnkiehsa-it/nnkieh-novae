...
- `organisms/DetailRouteState.vue` / `DetailPageShell.vue` / `SkeletonDetail.vue` — 三領域詳情共用的完整高度鏈、狀態與操作；桌面詳情採無外層卡片的平面雙欄，沿用父層可用高度與 `min-h-0` 捲動鏈以釋放正文／留言空間，不自行推算 `100dvh`；手機移除內容／留言 segmented tabs，以單一 scroll root 依序呈現正文、actions 與討論，留言 query 深連結先定位討論 anchor，再由留言串定位指定留言
...
- 留言：`CommentThreadPanel`、`CommentItem`、`CommentComposer`、`IssueComments`、`AnnouncementComments`、`UnavailableCommentDiscussion`；embedded 模式讓留言串沿用詳情外層捲動與 infinite-scroll root，手機 Composer 可由獨立 prop 固定在 Bottom Tab 上方；留言不開放時沿用同一 Composer 外觀並停用輸入、圖片、送出與自動 focus，設施只掛載無資料／無 service 的 unavailable presentation
