...
- `src/components/admin/` — admin overview/activity metrics, searchable user restriction management, audit-log presentation, category controls, scoped access, and platform retention/media-limit settings. Stateful admin-console reads and mutations stay in the domain hook; the user detail/restriction dialog is split from the searchable table.
...
- `cloudflare/src/backend/jobs/` — durable outbox, FCM/Notion delivery (including permanent management-audit archives with Notion formulas), deletion, realtime fan-out, and maintenance consumers driven by Cloudflare Queues.
...
- `database/migrations/` — fresh PostgreSQL 17 baseline and append-only migrations for Neon. `0002` bootstraps roles, `0003` completes realtime batches, `0004` seals the Worker-only database boundary, `0005` exposes scheduled support expiry, `0006` adds runtime settings, `0007` adds admin overview, user interaction restrictions, and audit-log data/functions, `0008` aligns admin user/activity visibility and adds audit-log retention cleanup, `0009` applies the complete retention lifecycle, `0010` adds permanent Notion activity archives, and `0011` seeds content versions from migration time so rebuilt databases invalidate every prior client cache.
