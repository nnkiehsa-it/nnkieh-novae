...
- `src/components/admin/` — admin overview/activity metrics, failed Cloudinary media-deletion recovery, searchable user restriction management, audit-log presentation, category controls, scoped access, full runtime retention/media settings, pre-save impact dialogs, and observable background-job progress. Stateful reads and mutations stay in domain hooks; repeated numeric/retention controls are shared components.
