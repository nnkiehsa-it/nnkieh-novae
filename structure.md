...
- `.github/workflows/deploy-frontend.yml` — Vercel Next.js build/deploy using `NEXT_PUBLIC_*` runtime names mapped from existing secret storage names, with environment-separated `.next/cache` restoration; frontend artifacts build before the matching backend deployment gate, while publication remains blocked until that backend deployment succeeds.
