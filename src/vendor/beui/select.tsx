...
export interface SelectTriggerProps {
  ...
  id?: string;
  "aria-label"?: string;
  "data-control-label"?: string;
}

export function SelectTrigger({
  className,
  children,
  "data-control-label": dataControlLabel = "",
  ...props
}: SelectTriggerProps) {
  ...
  return (
    <motion.button
      ...
      className={cn(
        "relative z-10 flex h-10 w-full items-center justify-between gap-2 rounded-xl border border-border bg-card px-3.5 py-2 text-foreground outline-none transition-colors",
...
export function SelectContent({ className, children }: SelectContentProps) {
  ...
  const open = ctx.open;
  const { placement, setPlacement, triggerId } = ctx;
  ...
  return (
    <motion.li variants={ctx.reduce ? undefined : ITEM_VARIANTS}>
      <button
        ...
        className={cn(
          "flex min-h-9 w-full items-center justify-between gap-2 rounded-xl px-2.5 py-1.5 text-left outline-none transition-colors",
          selected
