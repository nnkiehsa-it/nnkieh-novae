...
>(function AnimatedSidebarMenu(
  ...
  return (
    <SharedLayoutBg
      ...
      as="ul"
      inset={0}
      pillClassName="rounded-xl bg-[var(--surface-hover)]"
      pillContainerClassName="inset-y-1"
...
export function AnimatedSidebarMenuButton({
    ...
    <>
      {isActive ? (
        <motion.span
          ...
          className="absolute inset-x-0 inset-y-1 rounded-xl bg-[var(--nav-active-bg)]"
        />
      ) : null}
      {icon ? (
        <span
  ...
  const interactiveClassName = cn(
    ...
    "focus-visible:bg-muted/70 focus-visible:ring-2 focus-visible:ring-ring",
    isActive && "text-[var(--nav-active-fg)]",
    disabled && "cursor-not-allowed opacity-40",
    className,
  );
