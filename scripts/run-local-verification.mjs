...
function executable(name) {
  ...
  try {
    ...
    return decodeURIComponent(localPath.pathname.replace(/^\/([A-Za-z]:)/u, '$1'));
  } catch {
    return name;
  }
}
...
const steps = {
  ...
  checks: [
    ['TypeScript', executable('vue-tsc'), ['--noEmit']],
    ['unused declarations', executable('vue-tsc'), ['--noEmit', '--noUnusedLocals', '--noUnusedParameters']],
    ['translations', node, ['scripts/check-i18n.mjs']],
    ['UI architecture', node, ['scripts/check-ui-primitives.mjs']],
    ['ESLint', executable('eslint'), ['.', '--ext', '.ts,.vue']],
    ['production build types', executable('vue-tsc'), ['-b']],
    ['production bundle', executable('vite'), ['build', '--config', 'vite.config.ts']],
    ['build budget', node, ['scripts/check-build-budget.mjs']],
    ['Cloudflare Worker types', executable('tsc'), ['-p', 'cloudflare/tsconfig.json', '--noEmit']],
    ['Edge Function types', executable('deno'), [
      'check',
      '--no-lock',
  ...
  audit: [
    ['dependency audit', npmAuditCommand, npmAuditArgs],
  ],
};

const selectedSteps = suite === 'unit'
  ? [steps.tests[0]]
  : suite === 'architecture'
    ? [steps.tests[1]]
    : suite === 'test'
...
function startProgress(label) {
  if (!isInteractive) {
    process.stderr.write(`[${completed + 1}/${selectedSteps.length}] ${label}\n`);
    return;
  }
  const frames = ['⠋', '⠙', '⠹', '⠸', '⠼', '⠴', '⠦', '⠧', '⠇', '⠏'];
  let frame = 0;
  const render = () => {
    process.stderr.write(`\r\x1b[2K${formatProgress(`${frames[frame++ % frames.length]} ${label}`)}`);
  };
  render();
...
function warningLines(output) {
  ...
  const warningPattern = /\b(?:warning|warn|error|deprecated|ignored build scripts)\b/iu;
  return [...new Set(lines.filter((line) => warningPattern.test(line)))].slice(0, 40);
}
...
async function runStep([label, command, args]) {
  ...
  if (status !== 0) {
    ...
    if (!verbose && output.trim()) process.stderr.write(`${diagnosticTail(output)}\n`);
...
process.stderr.write(`✓ ${suiteLabel} passed (${selectedSteps.length} stages)\n`);
...
if (runAll) {
  await runSuite('[2/3] Integration verification', ['scripts/verify-integration-local.mjs']);
  await runSuite('[3/3] End-to-end verification', ['scripts/verify-integration-local.mjs', '--e2e']);
