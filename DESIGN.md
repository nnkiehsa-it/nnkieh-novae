...
- Discussion uses one continuous card surface with hairline-separated root comments and a single rail for nested replies. The avatar-led composer floats at the safe-area bottom as one bordered surface—its input row does not add nested frames—and submits through a compact circular up-arrow action. Replying adds a two-line target preview above it with the author, a 20-character excerpt, and an explicit cancel action. Reply groups start collapsed and expose an explicit count; root order is user-selectable while replies remain chronological.
...
- Route navigation commits directly without full-page snapshots or an outgoing layer. On both desktop and mobile, primary destinations selected from the sidebar or bottom navigation rise 12px; child routes enter 24px from the right; explicit or browser-history returns enter 24px from the left. Destination depth supplies the root/child default while an explicit return overrides it. Direction is captured once by the newly mounted pathname node and never mutated afterward, preventing delayed replay or horizontal bounce. All use the same restrained 2px blur over 350ms with no entrance delay. Dense feeds remain fully painted instead of replaying viewport-entry motion.
...
- Every cold route or local loading boundary hands off through the shared `SkeletonReveal` recipe: the final field owns layout while its lighter skeleton is absolutely stacked in the same slot. Only the exact text or numeric field sharpens from a bounded 2px blur over 400ms; there is no handoff translation. A cold skeleton remains visible for at least 280ms so a fast response cannot produce a one-frame flash. Fields restored from authoritative view/detail memory bypass the skeleton and reveal animation entirely.
- Category and status tags use the shared centered badge-label reveal, so cold skeleton width cannot change text alignment after cache restoration. Proposal categories that do not expose comments skip comment fetching and omit the complete discussion surface rather than rendering a disabled placeholder.
...
- iOS uses `viewport-fit=cover` with a translucent standalone status bar. The root document, startup fallback, manifest, and theme-color metadata share the stage surface so the app background continues beneath the status area without a white strip.
