...
- Product typography uses stable semantic roles across routes: 24px page titles, 24px mobile and 28px desktop detail titles, 16px/28px reading text, 16px card and section titles, 13px buttons, 13px metadata, and 13px segmented controls. Buttons, selects, menus, tabs, and segmented labels share `--control-label-size` while preserving their distinct geometry.
...
- On both desktop and mobile, primary destinations selected from the sidebar or bottom navigation use a native React View Transition crossfade: the outgoing route fades for 150ms while the incoming route resolves over 250ms, with no translation or blur. Browsers without View Transition support retain the incoming opacity fallback. Child routes still enter 24px from the right; explicit or browser-history returns enter 24px from the left. Destination depth supplies the root/child fallback while explicit navigation types and returns override it, preventing delayed replay or horizontal bounce. Spatial child/return routes retain restrained blur while dense feeds remain fully painted instead of replaying viewport-entry motion.
