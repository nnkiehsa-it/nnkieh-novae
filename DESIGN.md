...
- Product typography uses stable semantic roles across routes: 24px page titles, 24px mobile and 28px desktop detail titles, 16px/28px reading text, 16px card and section titles, 14px controls, 13px metadata, and 12px segmented controls.
...
- Primary actions use a dense, high-contrast treatment. Secondary and ghost variants preserve geometry and focus affordances.
...
- Liquid-gooey is limited to navigation and segmented/tab state movement, where the liquid material visibly explains selection continuity.
...
- Route content remounts on pathname changes with a short 250ms entrance while the persistent app shell stays anchored. Feed results stagger only when filters, sort, category, or committed search terms change.
...
- Touch targets remain at least 44px for primary mobile interactions.
