...
- Discussion uses one continuous card surface with hairline-separated root comments and a single rail for nested replies. The avatar-led composer floats at the safe-area bottom as one large-radius bordered surface—its input row does not add nested frames, its matching 40px avatar and circular up-arrow action share a bottom edge with symmetric outer inset, and growing text expands upward. Composer clearance belongs to the detail page bottom, never the discussion section itself, so mobile sidebar cards remain adjacent to the discussion card. Replying adds a two-line target preview above it with the author, a 20-character excerpt, and an explicit cancel action. Reply groups start collapsed and expose an explicit count; root order is user-selectable while replies remain chronological.
