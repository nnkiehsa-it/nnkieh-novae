...
- Discussion uses one continuous card surface with hairline-separated root comments and a single rail for nested replies. The avatar-led composer floats at the safe-area bottom; replying adds a two-line target preview above it with the author, a 20-character excerpt, and an explicit cancel action. Reply groups start collapsed and expose an explicit count; root order is user-selectable while replies remain chronological.
