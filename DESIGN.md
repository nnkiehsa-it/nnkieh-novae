...
- Feed re-entry keeps a cheap opacity/translate treatment on the item wrapper after its first settled paint. Initial data handoff never animates the item wrapper, so card borders, shadows, icons, progress, and action controls stay outside every filter layer.
...
- Every route or local loading boundary hands off through the shared `SkeletonReveal` recipe: the final field owns layout while its skeleton is absolutely stacked in the same slot. Only the exact text or numeric field sharpens from a bounded 2px blur over 400ms; there is no handoff translation. Buttons, reaction controls, icons, rails, card frames, borders, shadows, and surrounding layout never inherit this animation from a parent. This never replays route-scale movement, and feed cards do not stagger or translate during initial handoff.
