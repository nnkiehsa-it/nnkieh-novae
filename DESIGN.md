...
- Discussion uses one continuous card surface with hairline-separated root comments and a single rail for nested replies. The avatar-led composer floats at the safe-area bottom as one large-radius bordered surface—its input row does not add nested frames, its matching 40px avatar and circular up-arrow action share a bottom edge with symmetric outer inset, and growing text expands upward. Composer clearance belongs to the detail page bottom, never the discussion section itself, so mobile sidebar cards remain adjacent to the discussion card. The dock measures its live one-line, multiline, or reply-preview height and extends the page tail accordingly, allowing the final information card to scroll completely above it. Replying adds a two-line target preview above it with the author, a 20-character excerpt, and an explicit cancel action. Reply groups start collapsed and expose an explicit count; root order is user-selectable while replies remain chronological.
...
- Proposal support and facility affected actions use a line-hand reaction that presses and rises with an upward particle fan; announcement likes retain the filled-heart pop and radial particle burst. Every reaction control uses the same state hierarchy: inactive is a quiet ghost surface, active is a soft gray secondary surface, and its icon occupies a fixed 16px geometry box so icon-only and icon-plus-count variants share one optical center. Detail reaction actions are centered consistently. Reaction controls update state and count optimistically, keep their domain icon visible while the request is pending, and silently guard duplicate input without a disabled visual. A failed write rolls back the exact prior state and shows a domain-specific retry toast.
- Reaction counts animate per character without changing their inline width. Celebration particles are reserved for optimistic activation; removing a reaction changes state immediately without a spinner, check, or celebration.
...
- List entities use explicit summary types and list transports never include full content bodies. They may update shared title/status/reaction fields, but cannot satisfy a detail read or overwrite authoritative detail-only fields. A cold content route always reserves the full detail geometry and fetches the detail endpoint; only a previously fetched authoritative detail can render immediately without skeletons.
