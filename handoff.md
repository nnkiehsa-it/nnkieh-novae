...
- 本輪已修正實際的控制項、色彩與尺寸轉場問題；後續工作應以驗證實際問題為主，不再擴大設計或增加功能。
...
5. 明確內容狀態容器的高度變化使用 180ms resize，文字不被縮放，動畫能穩定結束；一般卡片、route 與 viewport reflow 不參與尺寸動畫。
...
- `src/components/motion/resize-motion.tsx`：只觀察明確 opt-in 的內容狀態容器，以高度-only 轉場處理 intrinsic size；視窗 reflow 立即重設量測，不會被當成內容動畫，並處理 reduced motion 與清理。
- `src/styles/motion.css`：Grid 採 `align-content: start`，避免容器縮放期間把卡片撐高造成回授；LiquidTabs 的非選取 rail 保留中性灰底，選取項使用較柔和的品牌色。
...
- 移除 AccentThemeProvider、AccentThemePicker、accent-theme 模組及獨立 ResizableCard。一般 Card 不做 intrinsic-size 動畫，只有必要的內容狀態容器使用共用 resize 機制。
...
- 最終 commit 尚未完整重跑 unit／architecture／tooling／production build／正式 E2E。
- 新增或更新的正式測試檔尚未全部執行確認：
  - `tests/e2e/feed-layout.spec.ts`
  - `tests/e2e/loading-continuity.spec.ts`
  - `tests/e2e/motion-system.spec.ts`
- 三個 ad hoc 案例不是所有頁面、裝置或失敗路徑的保證。
