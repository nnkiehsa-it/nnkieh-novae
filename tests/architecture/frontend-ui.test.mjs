...
test('entry and comment limits are enforced across UI, Edge, and a new migration', async () => {
  ...
  assert.match(commentComposer, /<Teleport to="body" :disabled="!mobileDocked">[\s\S]*comment-composer-dock/u);
...
test('primary navigation keeps desktop chrome and persistent mobile navigation', async () => {
  ...
  assert.match(app, /routeMotionInitial[\s\S]*opacity: 0,[\s\S]*route-forward[\s\S]*\? 22/u);
  assert.match(app, /routeMotionExit[\s\S]*opacity: 0,[\s\S]*route-forward[\s\S]*\? -14/u);
